...
In EPAAS, you can make accounting entries in multiple currency. For example, if you have a bank account in foreign currency, you can make transactions in that currency and system will show bank balance in that specific currency only.
...
### Exchange Rates
When dealing with multiple currencies, EPAAS has the Currency Exchange module for managing exchange rates. It allows you to save the exchange rate quotes you require. 

For foreign currency transactions, EPAAS checks Currency Exchange for any matching record. If this fails, EPAAS will attempt to get the exchange rate quote from [fixer.io](http://fixer.io). If this still fails, then the exchange rate will have to be entered manually.

#### Exchange Rate Selection
EPAAS automatically fetches the latest exchange rate available.
